...
// The maximum number of files to process in parallel.
var MAX_FILES_IN_PARALLEL = 72;
...
var compileHandle = function(poly, outputPath) {
 ...
 */
var copyFileHandle = function(publicPath, outputPath) {
  return function(file, done) {
    ...
    if(!polymer.helpers.shouldIgnore(file) && [".jade", ".ejs", ".md", ".styl", ".less", ".scss", ".coffee"].indexOf(ext) === -1){
      ...
      fs.mkdirp(path.dirname(localPath), function(err) {
        fs.copy(path.resolve(publicPath, file), localPath, done)
      });
    } else {
      done();
    }
 ...
 * Validate
 *
 * Compiles to a temp folder, returning errors or success
 *
 */

exports.validate = function(projectPath, callback) {
  ...
  helpers.ls(setup.publicPath, function(err, results){
    async.eachLimit(results, MAX_FILES_IN_PARALLEL, compileHandle(poly, outputPath), function(err){
      if(err){
        temp.cleanup();
        callback(err)
      }else{
    ...
    helpers.ls(setup.publicPath, function(err, results){
      async.eachLimit(results, MAX_FILES_IN_PARALLEL, compileHandle(poly, outputPath), function(err){
        if(err){
          ...
          async.eachLimit(results, MAX_FILES_IN_PARALLEL, copyFileHandle(setup.publicPath, outputPath), function(err){
